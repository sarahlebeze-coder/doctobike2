...
export default function BookingPage() {
  ...
  const Logo = () => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      ...
      <span style={{ fontFamily: "'Nunito', sans-serif", fontWeight: 700, fontSize: 22, color: '#042C53' }}>Doctobike</span>
    </div>
  )
  ...
  if (step === 4 && confirmed) return (
    <div style={{ minHeight: '100vh', background: '#F8FAFF' }}>
      ...
      <main style={{ maxWidth: 500, margin: '60px auto', padding: '0 20px', textAlign: 'center' }}>
        ...
        <h1 style={{ color: '#042C53', fontWeight: 700, fontSize: 24, marginBottom: 8 }}>Réservation confirmée !</h1>
        ...
        <button onClick={() => { setStep(1); setSelectedSlot(null); setConfirmed(null) }}
          style={{ background: 'none', border: '1px solid #ccc', padding: '10px 20px', borderRadius: 12, cursor: 'pointer', color: '#666', fontFamily: 'inherit' }}>
    ...
    <div style={{ minHeight: '100vh', background: '#F8FAFF' }}>
      ...
      {step === 1 && (
        <div style={{ background: 'linear-gradient(135deg, #042C53 0%, #185FA5 100%)', padding: '48px 20px', textAlign: 'center', color: 'white' }}>
          ...
          <h1 style={{ fontWeight: 700, fontSize: 30, marginBottom: 12, lineHeight: 1.3 }}>
            Réparez votre vélo<br />depuis chez vous
          </h1>
          <p style={{ color: '#B5D4F4', fontSize: 15, maxWidth: 420, margin: '0 auto' }}>
          ...
          <div>
            ...
            {Object.entries(byDate).sort().map(([date, daySlots]) => (
              <div key={date} style={{ background: 'white', borderRadius: 12, border: '1px solid #E6F1FB', padding: 20, marginBottom: 12 }}>
                <p style={{ fontWeight: 600, color: '#185FA5', marginBottom: 12, textTransform: 'capitalize', fontSize: 15 }}>
                  📅 {formatDate(date)}
                </p>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  {daySlots.map(slot => (
                    <button key={slot.id} onClick={() => { setSelectedSlot(slot); setStep(2) }}
                      style={{ padding: '10px 18px', borderRadius: 10, border: '1.5px solid #B5D4F4', background: '#E6F1FB', color: '#042C53', cursor: 'pointer', fontSize: 14, fontWeight: 600, fontFamily: 'inherit' }}>
        ...
        {step === 2 && selectedSlot && (
          <div style={{ background: 'white', borderRadius: 12, border: '1px solid #E6F1FB', padding: 24 }}>
            <div style={{ background: '#E6F1FB', borderRadius: 10, padding: 12, marginBottom: 20, textAlign: 'center' }}>
              <p style={{ color: '#185FA5', fontWeight: 600, margin: 0, textTransform: 'capitalize' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  <label style={{ fontSize: 13, color: '#666', display: 'block', marginBottom: 4, fontWeight: 500 }}>Prénom</label>
                  <input placeholder="Marie" value={form.firstName} onChange={e => setForm({ ...form, firstName: e.target.value })}
                    style={{ padding: '10px 12px', borderRadius: 8, border: '1px solid #ddd', fontSize: 14, width: '100%', fontFamily: 'inherit' }} />
                </div>
                <div>
                  <label style={{ fontSize: 13, color: '#666', display: 'block', marginBottom: 4, fontWeight: 500 }}>Nom</label>
                  <input placeholder="Laurent" value={form.lastName} onChange={e => setForm({ ...form, lastName: e.target.value })}
                    style={{ padding: '10px 12px', borderRadius: 8, border: '1px solid #ddd', fontSize: 14, width: '100%', fontFamily: 'inherit' }} />
                </div>
              </div>
              <div>
                <label style={{ fontSize: 13, color: '#666', display: 'block', marginBottom: 4, fontWeight: 500 }}>Email</label>
                <input placeholder="marie@exemple.fr" type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })}
                  style={{ padding: '10px 12px', borderRadius: 8, border: '1px solid #ddd', fontSize: 14, width: '100%', fontFamily: 'inherit' }} />
              </div>
              <div>
                <label style={{ fontSize: 13, color: '#666', display: 'block', marginBottom: 4, fontWeight: 500 }}>Décrivez votre panne</label>
                <textarea placeholder="Ex: Mon dérailleur arrière saute des vitesses, surtout en montée. Vélo de route, 21 vitesses..." value={form.description} onChange={e => setForm({ ...form, description: e.target.value })}
                  rows={4} style={{ padding: '10px 12px', borderRadius: 8, border: '1px solid #ddd', fontSize: 14, width: '100%', resize: 'vertical', fontFamily: 'inherit' }} />
              </div>
            </div>
            <div style={{ display: 'flex', gap: 8, marginTop: 20 }}>
              <button onClick={() => setStep(1)} style={{ padding: '10px 20px', borderRadius: 10, border: '1px solid #ddd', background: 'white', cursor: 'pointer', color: '#666', fontFamily: 'inherit' }}>← Retour</button>
              <button onClick={() => setStep(3)} disabled={!form.firstName || !form.lastName || !form.email}
                style={{ padding: '10px 20px', borderRadius: 10, border: 'none', background: '#185FA5', color: 'white', cursor: 'pointer', flex: 1, fontSize: 14, fontWeight: 600, fontFamily: 'inherit' }}>
                Continuer →
              </button>
            </div>
          </div>
          ...
          <div style={{ background: 'white', borderRadius: 12, border: '1px solid #E6F1FB', padding: 24 }}>
            ...
            <div style={{ background: '#F8FAFF', borderRadius: 10, padding: 16, marginBottom: 16 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #E6F1FB' }}>
                ...
                <span style={{ fontSize: 14, fontWeight: 600, textTransform: 'capitalize' }}>{formatDate(selectedSlot.date)}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #E6F1FB' }}>
                <span style={{ color: '#666', fontSize: 14 }}>Heure</span>
                <span style={{ fontSize: 14, fontWeight: 600 }}>{formatTime(selectedSlot.start_time)}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #E6F1FB' }}>
                <span style={{ color: '#666', fontSize: 14 }}>Client</span>
                <span style={{ fontSize: 14, fontWeight: 600 }}>{form.firstName} {form.lastName}</span>
              </div>
              {form.description && (
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 0 0', alignItems: 'baseline' }}>
                ...
                <span style={{ fontSize: 22, fontWeight: 700, color: '#185FA5' }}>45 €</span>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => setStep(2)} style={{ padding: '10px 20px', borderRadius: 10, border: '1px solid #ddd', background: 'white', cursor: 'pointer', color: '#666', fontFamily: 'inherit' }}>← Retour</button>
