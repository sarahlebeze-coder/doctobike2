...
function BookingForm() {
  ...
  if (step === 4 && confirmed) return (
    <div style={{ minHeight: '100vh', background: '#FAFCFA', fontFamily: font }}>
      <main style={{ maxWidth: 500, margin: '40px auto', padding: '0 20px', textAlign: 'center' }}>
        <div style={{ width: 64, height: 64, borderRadius: '50%', background: '#E8F5EE', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px', fontSize: 32, border: '2px solid #4DB892' }}>✓</div>
        <h1 style={{ color: '#1B4D3E', fontWeight: 700, fontFamily: font, marginBottom: 8 }}>Réservation confirmée !</h1>
        <p style={{ color: '#666', marginBottom: 16 }}>Un email de confirmation a été envoyé à <strong>{form.email}</strong></p>

        <div style={{ background: '#E8F5EE', borderRadius: 12, padding: 16, marginBottom: 8, textAlign: 'left' }}>
          <p style={{ color: '#2E8B6A', fontWeight: 700, margin: '0 0 4px', textTransform: 'capitalize' }}>📅 {formatDate(confirmed.date)}</p>
          <p style={{ color: '#2E8B6A', fontWeight: 600, margin: '0 0 4px' }}>🕐 {formatTime(confirmed.start_time)}</p>
          <p style={{ color: '#2E8B6A', fontWeight: 600, margin: 0 }}>📦 {selectedPack.name} — {selectedPack.label}</p>
        </div>

        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 20 }}>
          {confirmed.meet_link && (
            <a href={confirmed.meet_link} target="_blank" rel="noreferrer"
              style={{ display: 'block', background: '#1B4D3E', color: 'white', padding: '14px 20px', borderRadius: 12, textDecoration: 'none', fontSize: 15, fontWeight: 700 }}>
              🎥 Rejoindre la visio
            </a>
          )}
          <a href={buildGoogleCalendarUrl(confirmed.date, confirmed.start_time, confirmed.meet_link, selectedPack.duration)}
            target="_blank" rel="noreferrer"
            style={{ display: 'block', background: 'white', color: '#2E8B6A', padding: '14px 20px', borderRadius: 12, textDecoration: 'none', fontSize: 15, fontWeight: 700, border: '2px solid #4DB892' }}>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#FAFCFA', fontFamily: font }}>
      ...
      {step === 1 && (
        <div style={{ background: 'linear-gradient(135deg, #1B4D3E 0%, #2E8B6A 100%)', padding: '32px 20px', textAlign: 'center', color: 'white' }}>
          <p style={{ fontSize: 12, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.65)', marginBottom: 8, fontWeight: 600 }}>Assistance & dépannage vélo</p>
      ...
      <main style={{ maxWidth: 600, margin: '0 auto', padding: '24px 20px' }}>
        ...
        {step === 1 && (
          <div>
            <h2 style={{ color: '#1B4D3E', fontWeight: 700, fontSize: 18, marginBottom: 20 }}>Créneaux disponibles</h2>
            {Object.keys(byDate).length === 0 && (
              <div style={{ textAlign: 'center', padding: '40px 20px', color: '#888', background: 'white', borderRadius: 16, border: '1px solid #E8F5EE' }}>
                Aucun créneau disponible pour le moment.
              </div>
            )}
            {Object.entries(byDate).sort().map(([date, daySlots]) => (
              <div key={date} style={{ background: 'white', borderRadius: 16, border: '1px solid #E8F5EE', padding: 20, marginBottom: 12 }}>
                <p style={{ fontWeight: 700, color: '#2E8B6A', marginBottom: 12, textTransform: 'capitalize', fontSize: 15 }}>📅 {formatDate(date)}</p>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  {daySlots.map(slot => (
                    <button key={slot.id} onClick={() => { setSelectedSlot(slot); setStep(2) }}
                      style={{ padding: '10px 18px', borderRadius: 10, border: '2px solid #4DB892', background: '#E8F5EE', color: '#1B4D3E', cursor: 'pointer', fontSize: 14, fontWeight: 700, fontFamily: font }}>
        ...
        {step === 2 && selectedSlot && (
          <div style={{ background: 'white', borderRadius: 16, border: '1px solid #E8F5EE', padding: 24 }}>
            <div style={{ background: '#E8F5EE', borderRadius: 10, padding: 12, marginBottom: 20, textAlign: 'center' }}>
              <p style={{ color: '#2E8B6A', fontWeight: 700, margin: '0 0 4px', textTransform: 'capitalize' }}>📅 {formatDate(selectedSlot.date)} à {formatTime(selectedSlot.start_time)}</p>
              <p style={{ color: '#2E8B6A', fontWeight: 600, margin: 0, fontSize: 13 }}>📦 {selectedPack.name} — {selectedPack.label}</p>
            </div>
            <h2 style={{ color: '#1B4D3E', fontWeight: 700, fontSize: 18, marginBottom: 20 }}>Vos coordonnées</h2>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 20 }}>
              ...
              <button onClick={() => setStep(3)} disabled={!form.firstName || !form.lastName || !form.email}
                style={{ padding: '10px 20px', borderRadius: 10, border: 'none', background: '#1B4D3E', color: 'white', cursor: 'pointer', flex: 1, fontSize: 14, fontWeight: 700, fontFamily: font }}>
                Continuer →
              </button>
            </div>
          </div>
        ...
        {step === 3 && selectedSlot && (
          <div style={{ background: 'white', borderRadius: 16, border: '1px solid #E8F5EE', padding: 24 }}>
            <h2 style={{ color: '#1B4D3E', fontWeight: 700, fontSize: 18, marginBottom: 20 }}>Paiement sécurisé</h2>
            <div style={{ background: '#FAFCFA', borderRadius: 10, padding: 16, marginBottom: 16 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #E8F5EE' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #E8F5EE' }}>
                ...
                <span style={{ fontSize: 14, fontWeight: 700, color: '#2E8B6A' }}>{selectedPack.name}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0' }}>
                <span style={{ color: '#666', fontSize: 14 }}>Total</span>
                <span style={{ fontSize: 22, fontWeight: 800, color: '#1B4D3E', fontFamily: font }}>{selectedPack.price}€</span>
              </div>
            </div>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ padding: '12px 14px', borderRadius: 10, border: '1.5px solid #ddd', background: 'white' }}>
                <CardElement options={{ style: { base: { fontSize: '15px', color: '#1B4D3E', fontFamily: font, '::placeholder': { color: '#aaa' } } } }} />
              </div>
            </div>
...
export default function BookingPage() {
  return (
    <Suspense fallback={<div style={{ minHeight: '100vh', background: '#FAFCFA', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#2E8B6A', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>Chargement...</div>}>
      <BookingPageInner />
    </Suspense>
  )
}
